...
def normalize_clip(clip: Dict[str, Any], index: int) -> Dict[str, Any]:
    ...
    normalized = {
        ...
        "burned_caption_url": clip.get("burned_caption_url") or clip.get("edited_clip_url") or clip.get("preview_url"),
        ...
        "trend_alignment_reason": clip.get("trend_alignment_reason"),
        "reuse_potential": clip.get("reuse_potential"),
        "evergreen_status": clip.get("evergreen_status"),
        "time_sensitivity": clip.get("time_sensitivity"),
        "approval_state": clip.get("approval_state") or ("approved" if rendered and render_readiness_score >= 72 else "needs_edit" if not rendered else "needs_review"),
