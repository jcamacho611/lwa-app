...
async def enhance_clip_with_seedance(
    *,
    ...
    raise SeedanceProviderError(
        "Seedance clip enhancement is adapter-only until the exact vendor response contract is confirmed."
    )


def build_seedance_payload(*, settings: Settings, request: SeedanceBackgroundRequest) -> dict[str, Any]:
    payload = {
        # Keep the vendor-specific contract isolated here until the live API shape is verified.
