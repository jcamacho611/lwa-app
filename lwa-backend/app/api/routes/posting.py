...
def require_posting_unlocked(request: Request):
    user = require_user(request)
    require_feature_access(
        settings=settings,
        user=user,
        feature_name="posting_queue",
        detail="Upgrade to Scale to unlock posting connections and scheduled publishing.",
    )
    return user
...
async def list_connections(request: Request) -> dict[str, object]:
    user = require_posting_unlocked(request)
